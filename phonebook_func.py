...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tb1_phonebook (col_fname, col_lname, col_fullname, col_phone, col_email) VALUES (7, ?, ?, ?, ?) """, (data))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) : # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT (col_fullname) FROM thl_phonebook WHERE col_fullname = '()'""".format(var_fullname))#, (var_ful.
...
def onUpdate(self) :
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) : # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT (col_phone) FROM thl_phonebook WHERE col_phone = '{}'""".format (var_phone) )
            count = cur.fetchone()[0]
            print (count)
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                messagebox.showinfo ("No changes detected", "Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone,var_emaail))
            onClear (self)
        conn.close()
    else:
